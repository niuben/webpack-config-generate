			...
			fs.readFile("package.json", 'utf8', function(err, data){
				...
				try{
					...
					if(typeof content["dependencies"] == "object") {
						for(var i in content["dependencies"]) {
							dependencies[i] = content["dependencies"][i];						
						}
					}
	...
	inquirer.prompt([
		{
			...
			name: 'entry',
			message: function(a, b){
				return '请输入entry文件名（1/10）';
			},
		},{
		    type: 'input',
			name: 'output',
			message: function(a, b){
				return '请输入output文件夹名（2/10）'
			}
		},{
		    type: 'checkbox',
			name: 'style',
			message: function(a, b){
				return '选择支持样式格式 （3/10）';
			},
